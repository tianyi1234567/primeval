...
@OnlyIn(Dist.CLIENT)
public class YanexRenderer<T extends EntityAbstractSummonedSword> extends EntityRenderer<T> {
    private static final ResourceLocation TEXTURE = Primeval.prefix("model/util/yanex.png");
    private static final ResourceLocation MODEL = Primeval.prefix("model/util/yanex.obj");
    @Nullable
    public ResourceLocation getTextureLocation(T entity) {
        return entity.getTextureLoc();
    }

    public YanexRenderer(EntityRendererProvider.Context context) {
        super(context);
    }
    ...
    public void render(T entity, float entityYaw, float partialTicks, PoseStack matrixStack, MultiBufferSource bufferIn, int packedLightIn) {
        ...
        try {
            ...
            float scale = 0.0075F;
            if (entity instanceof SummonedSwordPlus summonedSwordPlus) scale*=summonedSwordPlus.getBaseSize();
