...
public class BIGswrod extends EntityAbstractSummonedSword
{
    ...
    public BIGswrod(EntityType<? extends Projectile> entityTypeIn, Level worldIn)
    {
    ...
    public static void doSlash(LivingEntity playerIn, int colorCode, boolean critical, double damage, float speed)
    {
        ...
        playerIn.getMainHandItem().getCapability(ItemSlashBlade.BLADESTATE).ifPresent((state) -> {
            ...
            {
                BigSumonswordEnity ss = new BigSumonswordEnity(PLEntiteRegristrys.bigm, worldIn);
