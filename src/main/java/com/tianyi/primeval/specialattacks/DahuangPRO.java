...
import com.tianyi.primeval.entity.YingYuEnity;
...
public class DahuangPRO extends EntityAbstractSummonedSword
{
    ...
    public DahuangPRO(EntityType<? extends Projectile> entityTypeIn, Level worldIn)
    {
    ...
    public static void doSlash(LivingEntity playerIn, int colorCode, boolean critical, double damage, float speed)
    {
        ...
        playerIn.getMainHandItem().getCapability(ItemSlashBlade.BLADESTATE).ifPresent((state) -> {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                double xOffset = random.nextDouble() * 10 * (isRight ? 10 : -10);
                double yOffset = random.nextFloat() * i * 0.005 + 12;
                double zOffset = random.nextFloat() * 10;
